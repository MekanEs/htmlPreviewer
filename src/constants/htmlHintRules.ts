  ...
  init(parser, reporter) {
    ...
    const onTagStart: Listener = event => {
      ...
      if (tagName === 'tr') {
        ...
        if (
          stack[stackLastIndex] !== 'table' &&
          stack[stackLastIndex] !== 'tbody' &&
          stack[stackLastIndex] !== 'thead'
        ) {
          reporter.error(
  ...
  init(parser, reporter) {
    ...
    const onTagEnd: Listener = event => {
      ...
      if (hasTitle && tagName === 'title') {
        ...
        if (!lastEvent) return;
        if (
          (lastEvent.type === 'text' && /[\w]/.test(lastEvent.raw ?? '') === true) ||
          titleStack[titleStack.length - 1] !== 'title'
        ) {
...
export const tagPairCustom: Rule = {
  id: 'tag-pair-custom',
  description: 'Tag must be paired.',
  init(parser, reporter) {
    const stack: { tagName: string; line: number; raw: string }[] = [];
